	...
	if (ev->window == root) {
		...
		if (updategeom() || dirty) {
			...
			for (m = mons; m; m = m->next) {
				...
				XMoveResizeWindow(dpy, m->extrabarwin, m->wx + esp, m->eby - evp, m->ww - 2 * esp, ebh);
			}
			focus(NULL);
			arrange(NULL);
		}
...
drawbar(Monitor *m)
{
	int x, w, ex, tw = 0, mw, ew = 0, etwl = 0, etwr = 0;
    ...
    if (n > 0) {
			...
			for (c = m->clients; c; c = c->next) {
				...
				if (tw > 0) /* trap special handling of 0 in drw_text */
					drw_text(drw, ex, 0, tw, ebh, lrpad / 2, c->name, 0);
				if (c->isfloating)
					drw_rect(drw, ex + boxs, boxs, boxw, boxw, c->isfixed, 0);
				ex += tw;
        w -= tw;
			}
		}
		drw_setscheme(drw, scheme[SchemeNorm]);
		drw_rect(drw, ex, 0, w - 40, ebh, 1, 1);
	}

	drw_map(drw, m->extrabarwin, 0, 0, m->ww, ebh);
	drw_rect(drw, 0, 0, m->ww, ebh, 1, 1);
  ...
  if (m == selmon) { /* extra status is only drawn on selected monitor */
		...
		etwl = TEXTW(estextl);
		drw_text(drw, 5, 0, etwl, bh, 0, estextl, 0);
	}
}
...
setup(void)
{
	...
	lrpad = drw->fonts->h;
	bh = (drw->fonts->h + 2) + barborderpx * 2;
  ebh = (drw->fonts->h + 2) + ebarborderpx * 2;
	sp = sidepad;
	vp = (topbar == 1) ? vertpad : - vertpad;
	esp = esidepad;
	evp = (extrabar == 1) ? evertpad : - evertpad;
	updategeom();
	/* init atoms */
...
tile(Monitor *m)
{
	...
	for (i = my = ty = 0, c = nexttiled(m->clients); c; c = nexttiled(c->next), i++)
		if (i < m->nmaster) {
			...
			resize(c, m->wx, m->wy + my, mw - (2*c->bw) + (n > 1 ? gappx : 0), h - (2*c->bw) - vertpad - evertpad, 0);
			if (my + HEIGHT(c) < m->wh)
				my += HEIGHT(c);
		} else {
			h = (m->wh - ty) / (n - i);
			resize(c, m->wx + mw, m->wy + ty, m->ww - mw - (2*c->bw), h - (2*c->bw) - vertpad - evertpad, 0);
			if (ty + HEIGHT(c) < m->wh)
				ty += HEIGHT(c);
		}
}
...
toggleextrabar(const Arg *arg)
{
	...
	updatebarpos(selmon);
	XMoveResizeWindow(dpy, selmon->extrabarwin, selmon->wx - esp, selmon->eby - evp, selmon->ww - 2 * esp, bh);
	arrange(selmon);
}
...
updatebars(void)
{
	...
	for (m = mons; m; m = m->next) {
		...
		if (!m->extrabarwin) {
			m->extrabarwin = XCreateWindow(dpy, root, m->wx + esp, m->eby - evp, m->ww - 2 * esp, ebh, 0, DefaultDepth(dpy, screen),
					CopyFromParent, DefaultVisual(dpy, screen),
					CWOverrideRedirect|CWBackPixmap|CWEventMask, &wa);
...
updatebarpos(Monitor *m)
{
	...
	if (m->extrabar) {
		m->wh -= bh;
		m->eby = !m->topbar ? m->wy : m->wy + m->wh - evertpad;
		m->wy = !m->topbar ? m->wy + bh - evp : m->wy;
	} else
		m->eby = -bh + vp;
}
